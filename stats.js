  ...
  function renderTableRuns(allRuns, inProgress) {
    ...
    const rows = [...allRuns].map((r, i) => ({ r, i, cl: cpmLabels[i], el: errLabels[i], lc: lvlChanges[i] })).reverse().map(({ r, i, cl, el, lc }) => {
      ...
      <tr${r.lazy ? ' class="row--lazy"' : ''} data-run-key="${r.date}~${r.time ?? ''}">
        ...
        <td title="Предложение ${r.sentenceStart ?? '?'}, всего ${r.sentenceCount ?? '?'} пр.">${r.textSet ?? 1}</td>
    ...
    const inProgressRow = inProgress ? (() => {
      ...
      <tr class="row--in-progress">
        ...
        <td title="Предложение ${inProgress.sentenceStart ?? '?'}, всего ${inProgress.sentenceCount ?? '?'} пр.">${inProgress.textSet ?? 1}</td>
